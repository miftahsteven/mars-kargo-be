...
export class EmailService {
  ...
  public static async sendActivationEmail(params: {
    toEmail: string;
    toName: string;
    nip: string;
    password: string;
    ...
    const activationUrl = `${ENV.BASE_URL}/api/auth/activate?token=${params.activationToken}`;
    ...
    try {
      console.log(`[EmailService] Sending activation email via Nodemailer (${ENV.SMTP_USER}) to ${params.toEmail}...`);
      const info = await this.transporter.sendMail({
        from: `"MarsCargo System" <${ENV.SMTP_USER}>`,
        to: params.toEmail,
